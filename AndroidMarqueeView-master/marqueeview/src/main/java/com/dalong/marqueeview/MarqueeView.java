...
//        mTextPaint.setShadowLayer(5, 3, 3, ShadowColor);
    ...
    public void setSizeAndColor(String mTextSize, String mTextColor) {
      ...
      /*  String hex = mTextColor;
        int mTextColorint = (int) Long.parseLong(hex, 16);*/
        ...
        this.mTextColor = Color.parseColor(mTextColor);
        // this.mTextColor = mTextColorint;
    }
            ...
            try {
                synchronized (holder) {
                    ...
                    if (mDirection == 0) {//向左滚动
                        ...
                        Log.e("currentX", "currentX" + currentX + "contentWidth" + contentWidth);
                        if (currentX >= contentWidth) {
                            if (!mIsRepeat) {//如果是不重复滚动
                                mHandler.sendEmptyMessage(ROLL_OVER);
                            }
                            currentX = -textWidth;
                            Log.e("currentX11", "currentX11");
                        } else {
                            currentX += sepX;
                            Log.e("currentX22", "currentX22");
                        }
                    }
